<template>
  <div class="container">
    <div class="section">
      ...
      <div class="percentage" :style="{ width: progressPercentage + '%' }"></div>

      <div class="section-container">
        <div class="step" v-show="step === 1">
          <div class="select-box" v-for="(item, index) in items" :key="index" v-on:click="select(item.h1)">
            <h1>{{ item.h1 }}</h1>
            <h3>{{ item.h3 }}</h3>
          </div>
        </div>
        ...
        <div class="step" v-show="step === 2">
          <div class="order-field">
            <label>
              <input type="text" id="company" v-model="form.company" placeholder="Company" />
            </label>
          </div>
          <div class="order-field">
            <label>
              <input type="text" id="forename" v-model="form.forename" placeholder="First name *" />
            </label>
          </div>
          <div class="order-field">
            <label>
              <input type="text" id="surname" v-model="form.surname" placeholder="Last name *" />
            </label>
          </div>
          <div class="order-field">
            <label>
          ...
          <div class="order-field">
            <label>
              <input type="text" id="number" v-model="form.number" placeholder="Phone number" />
            </label>
          </div>
        </div>
        <div class="step" v-show="step === 3">
          <div :style="{ background: form.scheme }" class="select-scheme">
            <div class="color-picker">
              <ColorPicker style="width: 218px" theme="light" :color="form.scheme" :sucker-hide="true"
        ...
        <div class="step" v-show="step === 4">
          <div class="order-field">
            <label>
              <textarea v-model="form.imagination" type="text"
                placeholder="Write a brief overview of the website you would like us to create for you."></textarea>
            </label>
          </div>
        </div>
...
.percentage {
  ...
  -webkit-mask-image: -webkit-linear-gradient(320deg,
      rgba(55, 0, 255, 0.8) 70%,
      rgba(255, 0, 25, 1) 75%,
      rgba(55, 0, 255, 0.8) 95%);
  mask-image: linear-gradient(320deg,
      rgba(55, 0, 255, 0.8) 70%,
      rgba(255, 0, 25, 1) 75%,
